...
export default async function (cliConfig: Cli) {
  ...
  if (cloud?.import) {
    tasks.push(
      fsPromises.writeFile(
        `${outputPath}/import.json`,
        JSON.stringify(cloud.import)
      )
    );
  }
...
async function upload(params: {
  ...
  const { filename, component_id, package_name, package_version } = params;
